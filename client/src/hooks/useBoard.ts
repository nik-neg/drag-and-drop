...
export const useBoard = ({ data, handleSetData, boardStateRef, registry, triggerPostMoveFlash, liveRegion, instanceId }: IUseBoardProps) => {

    const { lastOperation } = data;

    const { reorderColumn, reorderCardInSameColumn, moveCardToNewColumn, handleLastOperation } = useCallbacks();
